...
当前 NALU IPC DATAFIFO 参数按 SDK VENC DATAFIFO 对齐：`3` 个 entry，每个 item `512` 字节，必须和大核 writer 侧 `u32EntriesNum` / `u32CacheLineSize` / `mpp_nalu_ipc_msg` layout 完全一致。小核代码在 `nalu_datafifo.h` 中用 `NALU_DATAFIFO_EXPECTED_ITEM_SIZE` 做编译期检查；如果外部 `mpp_nalu_ipc.h` 定义成其他 item size，会直接编译失败。大小核 `mpp_nalu_ipc_msg` 字段也必须一致，当前小核结构包含大核侧的 `submit_time_ms` 字段。
...
/home/ubuntu/k230_sdk/toolchain/Xuantie-900-gcc-linux-5.10.4-glibc-x86_64-V2.6.0/bin/riscv64-unknown-linux-gnu-gcc \
  ...
  -o rtsp_sender \
  RTSP_Create.c rtsp.c rtp.c file.c nalu_datafifo.c snapshot_writer.c \
  -L/home/ubuntu/k230_sdk/src/common/cdk/user/component/datafifo/slave/lib \
  -ldatafifo -lpthread
```

## DATAFIFO snapshot trigger

Snapshot capture now uses the existing `mpp_nalu_ipc_msg.reserved` field; no
DATAFIFO item size or message layout change is required.

Big core contract:

```c
msg.reserved |= MPP_NALU_IPC_FLAG_SNAPSHOT; /* bit0, value 0x1 */
```

When the little core receives a valid DATAFIFO message with bit0 set, it maps
all packs in that message before `DATAFIFO_CMD_READ_DONE`, copies the encoded
frame payload into the asynchronous snapshot writer queue, and saves it under:
...
The snapshot writer creates files with `O_EXCL`, so existing snapshots are not
overwritten even if the process restarts and the sequence starts from 1 again.
If the queue is full, the snapshot request is dropped and RTP sending continues.
## 6. 推荐测试顺序
